...
echo bsppath - %bsppath%
echo mapsourcepath - %mapsourcepath%
echo treepath - %treepath%
echo mapname - %mapname%
echo parmBSPOptions - %parmBSPOptions%
echo parmLightOptions - %parmLightOptions%
echo compileBSP - %compileBSP%
echo compileLight - %compileLight%
...
if "%compileBSP%" == "1" (
	...
	"%treepath%bin\cod4map" -platform pc -loadFrom "%mapsourcepath%%mapname%.map" -v -verboseEntities %parmBSPOptions% "%bsppath%%mapname%"
)
...
if "%compileLight%" == "1" (
	...
	"%treepath%bin\cod4rad" -platform pc -Traces 512 %parmLightOptions% "%bsppath%%mapname%"
)
...
if "%compilePaths%" == "1" (
	...
	cd %treepath%
	rem sp_tool.exe +set r_fullscreen 0 +set logfile 2 +set monkeytoy 0 +set com_introplayed 1 +set usefastfile 0 +set g_connectpaths 2 +devmap %mapname%
