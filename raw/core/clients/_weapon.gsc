...
/*
GiveSecondaryWeapon( weaponFile, switchWeapon )
{
	if( weaponFile == "" )
		return;

	self.SpawnPlayer.secondaryWeapon = weaponFile;
	self GiveWeapon( weaponFile );
	
	if( IsDefined( switchWeapon ) && switchWeapon )
		self thread SwitchWeapon( weaponFile );
		
	scripts\_events::RunCallback( level, "onGiveSecondaryWeapon", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "onGiveSecondaryWeapon", 1, weaponFile );
	self OnGiveWeapon( weaponFile );
}
GivePrimaryWeapon( weaponFile, switchWeapon )
{
	if( weaponFile == "" )
		return;

	self.SpawnPlayer.primaryWeapon = weaponFile;
	self GiveWeapon( weaponFile );
	
	if( IsDefined( switchWeapon ) && switchWeapon )
		self thread SwitchWeapon( weaponFile );
		
	scripts\_events::RunCallback( level, "onGivePrimaryWeapon", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "onGivePrimaryWeapon", 1, weaponFile );
	self OnGiveWeapon( weaponFile );
}
GiveGrenade( weaponFile )
{
	if( weaponFile == "" )
		return;

	self.SpawnPlayer.offHand = weaponFile;
	self GiveWeapon( weaponFile );
	
	scripts\_events::RunCallback( level, "onGiveGrenade", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "onGiveGrenade", 1, weaponFile );
	self OnGiveWeapon( weaponFile );
}
GiveSpecialGrenade( weaponFile )
{
	if( weaponFile == "" )
		return;

	self.SpawnPlayer.secondaryOffHand = weaponFile;
	self GiveWeapon( weaponFile );
	
	if( weaponFile == "flash_grenade_mp" )
		self SetOffhandSecondaryClass( "flash" );
	else if( weaponFile == "smoke_grenade_mp" )
		self SetOffhandSecondaryClass( "smoke" );
		
	scripts\_events::RunCallback( level, "onGiveSpecialGrenade", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "onGiveSpecialGrenade", 1, weaponFile );
	self OnGiveWeapon( weaponFile );
}

RestoreAmmo( weaponFile )
{
	self GiveStartAmmo( weaponFile );
	
	scripts\_events::RunCallback( level, "restoreAmmo", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "restoreAmmo", 1, weaponFile );
}

GetGrenadeType( weaponFile )
{
	switch( weaponFile )
	{
		...
		default:
			PrintDebug( "Unknown grenade type ^1"+weaponFile );
			return "";
	}
}
...
OnGiveWeapon( weaponFile )
{
	scripts\_events::RunCallback( level, "onGiveWeapon", 1, self, weaponFile );
	scripts\_events::RunCallback( self, "onGiveWeapon", 1, weaponFile );	
}
*/
InSVStartGameType()
{
...
//	thread maps\mp\_ied::main();
	//thread scripts\_flashgrenades::main();
...
onPlayerSpawned()
{
	...
	self thread CMonitorWeaponChanging();
}

// callback
 CMonitorWeaponChanging()
{
	self endon("death");
	self endon("disconnect");
	
	sLast = self GetCurrentWeapon();
	
	while (true)
	{
		self waittill("weapon_change", sNew);

		if (self CCheckActionSlots(sNew))
		{
			self SwitchToWeapon(sLast);
			continue;
		}
		
		self COnWeaponChange(sNew, sLast);
		sLast = sNew;
	}	
}

 COnWeaponChange(sNew, sLast)
{
	// call callbacks
	args = core\include\_eventCallback::Args_Create();
	args.sLastWeapon = sLast;
	args.sNewWeapon = sNew;
	self core\_init::RunOnCWeaponSwitch(args);
}

CCheckActionSlots(sNew)
{
	switch (sNew)
	{
		case "radar_mp": // PERK BUTTON
			args = core\include\_eventCallback::Args_Create();
			self core\_init::RunOnCPerkBtnPressed(args);
			return true;
		default:
			return false;
	}
...
watchWeaponUsage()
{
	...
	{	
		...
		//scripts\_events::RunCallback( level, "begin_firing", 1, self, curWeapon );
		//scripts\_events::RunCallback( self, "begin_firing", 1, curWeapon );
		...
		//scripts\_events::RunCallback( level, "end_firing", 1, self, curWeapon );
		//scripts\_events::RunCallback( self, "end_firing", 1, curWeapon );
	}
}
...
setupBombSquad()
{
	...
	if ( self HasPerk( "specialty_detectexplosive" ) && !self.bombSquadIcons.size )
	{
	...
	else if ( !self HasPerk( "specialty_detectexplosive" ) )
	{
