main()
{
	...
	end_sniperarea=getentarray("end_sniperarea","targetname");
	if(isdefined(end_sniperarea))
	{
	for(i=0;i<end_sniperarea.size;i++)
	{
	end_sniperarea[i] thread snipergame();
	}
	}
	...
	thread ak74u_trigger();
	thread weaponroommessage();
	thread trap_spinner();
	thread oldgame();
	thread jumpgame();
	thread movejump1();	
...
antibug()
{
	...
	trig = spawn( "trigger_radius", (2791,-473,536) , 0, 300, 300); //Spawn( <classname>, <origin>, <flags>, <radius>, <height> )
...
{
iPrintLnBold("This door is locked"); //Change the message if you want
}
else
{
level.sniper_trigger waittill("trigger", player);

player deathrun\include\_mapper::CFinishMap();
...
iPrintLnBold("^1 " + player.name + "^7 Has picked ^2Sniper Game!"); //Change the message if you want
...
finaldoor = getEnt("finaldoor", "targetname");
finaldoor moveZ( -210,2);
iPrintLnBold("^1 " + player.name + " ^7Has picked ^2Classic!"); //Change the message if you want
}
...
{
actdoor movez(-300,3);
iPrintLnBold("^1Activators ^7Gamedoor Unlocked !"); //Change the message if you want
wait(12);
jumperdoor movez(-300,3);
}
else
{
iPrintLnBold("^4" + player.name + "^3 is way faster then the activator XD"); //Change the message if you want
}
}
...
jumperdoor movez(-300,3);
iPrintLnBold("^2Jumpers ^7Gamedoor Unlocked !"); //Change the message if you want
wait(12);
actdoor movez(-300,3);
}
...
door()
{
	...
	iPrintLnBold(player.name + "^3 has entered the hallway!"); //Change the message if you want
}
...
walljump()
{
	...
	trigger waittill("trigger", player);
	iPrintLnBold(player.name + "^3 has found a shortcut !"); //Change the message if you want
	wait(1);
...
{
iPrintLnBold(player.name + "^6 is a pussy trying to activate on free !"); //Change the message if you want
}
else
{
while(1)
...
secretshaft()
	...
	if( isDefined( level.trapsDisabled ) && level.trapsDisabled )
	{
	iPrintLnBold(player.name + "^3 isnt strong enough to open the shaft !"); //Change the message if you want
	}
	else
	{
	trigger delete();
...
weaponroom()
{
	...
	trigger waittill("trigger", player);
	iPrintLnBold(player.name + "^3 has made ^1THE PERFECT HEADSHOT!"); //Change the message if you want
	rpgroom movex (-600,1);
	trigger delete();
	iPrintLnBold("A wall has moved ?"); //Change the message if you want
}
...
trigger_trap7 waittill ("trigger", player);
if( isDefined( level.trapsDisabled ) && level.trapsDisabled && player == level.activ )
{
iPrintLnBold(player.name + "^6 is a pussy trying to activate on free !"); //Change the message if you want
}
