...
finishedMap()
{
	...
	if( level.jumpers_finished == 1 )
	{
		...
		self deathrun\include\_mapper::CFinishMap();
	}
...
playersInRoom( player )
{
	...
	if( isDefined( player.slow_noob ) && player.slow_noob )
	{
		...
		player setMoveSpeedScale( deathrun\include\_mapper::SVJSpeedGet() );
	}
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		if( players[i] != level.activ )
		{
			...
			if( isDefined( players[i].slow_noob ) && players[i].slow_noob )
			{
				...
				players[i] setMoveSpeedScale( deathrun\include\_mapper::SVJSpeedGet() );
			}
