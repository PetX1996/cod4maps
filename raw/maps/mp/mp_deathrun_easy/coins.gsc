	...
	switch( item )
	{
		case 0:
			if( level.mapmode != 3 )
			{
				...
				iprintlnbold( "Player ^1"+ self.name +"^7 found ^5MapMode ^1DOWN^7." );
			}
		...
		case 2:
			if( level.mapmode != 1 )
			{
				level.mapmode--;
				level notify( "change_map_mode" );
				iprintlnbold( "Player ^1"+ self.name +"^7 found ^5MapMode ^1UP^7." );
			}
			
			break;			
		case 3:
			//speed = level.dvar["allies_speed"]*1.2;
			speed = deathrun\include\_mapper::SVJSpeedGet() * 1.2;
		...
		case 5:
			checkpoint = origin;
			lifes = self deathrun\include\_mapper::CLifesGetCount();
		...
		case 50:
			...
			iprintlnbold( "Player ^1"+ self.name +"^7 found ^1KEY ^2to the last room^7." );
			break;
		default:
...
CheckPointSystem( checkpoint, lifes )
{
	...
	if (self deathrun\include\_mapper::CLifesGetCount() < lifes)
	{
