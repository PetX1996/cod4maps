...
trap1_player(player)
{
	...
	fx = PlayFX( level._effect[ "blesk" ], player.origin );
...
trap8()
{
	...
	Earthquake( 1, 4, trig.origin, 300 );
	
	model movey(int(dist),4);
...
on_touch()
{
	...
	{
		self waittill("trigger", player);
		player FinishPlayerDamage(player, player, 50, 0, "MOD_PROJECTILE", "rpg_mp", self.origin, vectornormalize(player.origin - self.origin), "none", 0);
	}
}
...
move_raptor(target_point,trig)
{
	...
	self moveto(target_point,2.5); //druhy udaj rychlost
...
trap10()
{
	...
	trig = getent("t11k","targetname");
	model = getent("t11","targetname");
	target_dis = getent(model.target,"targetname");
	
	startorig = model.origin;
	endorig = (target_dis.origin[0], target_dis.origin[1], model.origin[2]);
	...
	model moveto(endorig, 1.5);
	
	model waittill("movedone");
	wait 1;
	
	model moveto(startorig, 2);
	model waittill("movedone");
