...
//maps\mp\_compass::setupMiniMap("compass_map_mp_dr_bananaphone");
...
iPrintLnBold("^3The ^1bananaphone ^3got picked up by ^1"+player.name+" ^3WOOT:D");
...
//iPrintLnBold("The elevator is already on floor 1");
}
wait(0.01);
} 
}
...
//iPrintLnBold("The elevator is already on floor 2");
}
wait(0.01);
}
}
...
//iPrintLnBold("The elevator is already on floor 3");
}
wait(0.01);
}
}
...
level.busy = true;
iPrintLnBold("^1"+player.name+" ^3entered the magical ^1banana ^3room");
level.secret = true;
if(level.floor == 1){
deur1 moveY(-78,0.5);
deur3 moveY(-78,0.5);
...
iPrintLnBold("^3The ^1bananaphone ^3is ^1ringing! ^3Pickup the ^1phone!");
door moveZ (192, 4);
...
antibug()
{
	...
	trig = spawn( "trigger_radius", (2110,1507,370) , 0, 130, 185); //Spawn( <classname>, <origin>, <flags>, <radius>, <height> )
	...
	while(1)
	{
		...
		trig = spawn( "trigger_radius", (2281,p,243) , 0, 15, 211); //Spawn( <classname>, <origin>, <flags>, <radius>, <height> )
		trig thread start_sec();	
		p += 30;
	}
}
