...
main()
{
	...
	thread antibug_Initialize();
}

//------------------Antibug system by 4GF|Kyry------------------------------
antibug_Initialize()
{ 
 areas=[];
 areas[0]=(1488.42,771.55,-2891.88);//Most
 for (i=0;i<areas.size;i++)
     thread antibug_CheckPlayers(areas[i]);
}
antibug_CheckPlayers(origin)
{
 while (true)
  {
   	triggerRadius = spawn("trigger_radius", origin, 0, 50, 200);
   	triggerRadius waittill( "trigger", player );
   	player iprintlnbold("^5Bug FIXED :-)\n^14GF Team");
    player suicide();   
  }
}
...
WatchStairs()
{
	while(1)
	{
		...
		{
			who suicide();
			who iprintlnbold("^1DONT TRY TO KILL YOUR MATES! FUCK YOU!");
			continue;
		}
		thread DoStairs();
		self delete();
...
WatchStartTrigger()
{
	...
	while(1)
	{
		...
		{
			who suicide();
			who iprintlnbold("Sorry, but no respawn possible. :3");
		}
	}
}
...
BurnPlayer()
{
	...
	self suicide();
	iprintln( self.name + " ^1burned down!");
}
